...
**`compile.sh`** - This script automates the full BusyBox compilation process. First, it enables strict Bash error handling and checks if the script is run with root privileges, because it needs permission to work inside `/opt/task2`. Then it defines all important paths, BusyBox version information and download links. The script checks whether required build tools such as `wget`, `tar`, `python3`, `gcc` and `make` are installed, and if some tools are missing, it installs them automatically using `apt`. After that, it creates the `src` directory, downloads the BusyBox archive if it is not already present, and extracts the source files. The script enters the BusyBox source directory, removes old build files, creates a default configuration, enables static compilation and disables `CONFIG_TC` to avoid possible build problems. Then BusyBox is compiled using all available CPU cores. Before replacing the old binary, the script stops the `bb-httpd` service to avoid file conflicts. Finally, the compiled BusyBox binary is copied to `/opt/task2`, execute permissions are added, a small functionality check is performed using `--help`, and the script prints a message that compilation has finished.

**`deploy.sh`** -this script deploys the compiled BusyBox binary and prepares the HTTP service. First, it checks that the script is run with root privileges and verifies that `/opt/task2/busybox` exists and is executable. Then it gets the list of all BusyBox commands and creates wrapper files in `/bin` with names like `bb-cat`, `bb-ls` and `bb-httpd`. Each wrapper simply runs the main BusyBox binary with the correct command name, so every BusyBox utility can be used as a separate command. After that, the script creates the web server directory `/var/www/html` and writes a simple `index.html` file with the text _I am alive empo1010_. It also creates an empty BusyBox HTTP configuration file in `/etc/bb-httpd.conf`. Then the script writes a systemd service file for `bb-httpd`, where the server is configured to run on port `80`, serve files from `/var/www/html` and restart automatically if it crashes. Finally, the script reloads systemd, enables the service to start automatically after reboot, and starts the BusyBox HTTP server immediately.

**`test.sh`** - this script tests the deployed BusyBox commands. First, it checks that the script is run with root privileges, because some tests need higher permissions. Then it defines colors and counters for passed, failed and skipped tests. The main check function runs a given command, compares its exit code with the expected result, and prints PASS or FAIL. The script also remembers which commands were already tested, so they are not tested twice. After that, it runs many real functional tests for common BusyBox commands, such as `bb-ls`, `bb-cat`, `bb-cp`, `bb-mkdir`, `bb-rm`, `bb-ping`, `bb-tar`, `bb-sort` and others. Most tests use safe temporary files in `/tmp` and remove them after testing. For `bb-ping`, the script adds the needed network capability to the BusyBox binary, so ping can work without permission errors. After the main functional tests, the script goes through all `/bin/bb-*` wrapper commands that were not tested yet and checks that their `--help` option works. At the end, it prints a summary with the number of passed functional tests, failed commands and commands that were only checked with `--help`.

**`httptest.sh`** - This small script checks if the BusyBox HTTP server is working. It uses `bb-wget` to send a request to `http://localhost`. The `-q` option makes the command quiet, and `-O-` prints the downloaded page content to the terminal instead of saving it to a file. If the server is running correctly, it should return the content of the `index.html` page.

**`install_service.sh`** - this script is the main pipeline script that runs the full installation and testing process. First, it checks that it is started with root privileges, because all other scripts need permission to compile, deploy files and manage the service. Then it moves to the `/opt/task2` directory and runs all project scripts in the correct order. It first runs `compile.sh` to download, configure and compile BusyBox. After that, it runs `deploy.sh` to create BusyBox command wrappers and set up the `bb-httpd` service. Then it runs `test.sh` to check if the BusyBox commands work correctly. Finally, it runs `httptest.sh` to check if the HTTP server responds on localhost. If all steps finish without errors, the script prints a final message that installation and testing finished successfully.
